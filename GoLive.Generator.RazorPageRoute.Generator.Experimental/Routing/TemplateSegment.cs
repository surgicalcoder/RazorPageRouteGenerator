...
namespace GoLive.Generator.RazorPageRoute.Generator.Experimental.Routing;
...
internal class TemplateSegment
{
    public TemplateSegment(string template, string segment, bool isParameter)
    {
        ...
        IsCatchAll = isParameter && segment.StartsWith('*'.ToString());
        ...
        if (IsParameter)
        {
            ...
            else
            {
                ...
                Value = tokens[0];
                IsOptional = tokens[tokens.Length - 1].EndsWith('?'.ToString());
                if (IsOptional)
                {
    ...
    public override string ToString() => this switch
    {
        ...
        { IsParameter: true, IsOptional: false, IsCatchAll: false, Constraints: { Length: > 0 } } => $"{{{Value}:{string.Join(':'.ToString(), (object[])Constraints)}}}",
        { IsParameter: true, IsOptional: true, Constraints: { Length: 0 } } => $"{{{Value}?}}",
        { IsParameter: true, IsOptional: true, Constraints: { Length: > 0 } } => $"{{{Value}:{string.Join(':'.ToString(), (object[])Constraints)}?}}",
        { IsParameter: true, IsCatchAll: true, Constraints: { Length: 0 } } => $"{{*{Value}}}",
        { IsParameter: true, IsCatchAll: true, Constraints: { Length: > 0 } } => $"{{*{Value}:{string.Join(':'.ToString(), (object[])Constraints)}?}}",
